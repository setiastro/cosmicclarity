...
class SatelliteToolDialog(QDialog):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Cosmic Clarity Satellite Removal Tool V1.1")
...
def satellite_image(image_path, device, detection_model, removal_model, satellite_mode='luminance', progress_signal=None, log_signal=None):
    ...
    try:
        ...
        # Check if file extension is '.xisf'
            ...
            log_message(f"Loaded XISF image with bit depth: {bit_depth}, Mono: {is_mono}")
        ...
        satellited_image = remove_border(satellited_image, border_size=16)
        ...
        return satellited_image, original_header, bit_depth, file_extension, is_mono, file_meta
...
def satellite_channel(image, device, removal_model, detection_model, is_mono=False, progress_signal=None):
    ...
    for idx, (chunk, i, j) in enumerate(chunks):
        ...
        if GLOBAL_CLIP_TRAIL:
            ...
            binary_mask = np.where(clipped_image < 0.5, 0, 1)
...
def save_processed_image(image, output_path, file_extension, bit_depth, original_header=None, is_mono=False, file_meta=None):
    ...
    if file_extension in ['.fits', '.fit']:
        ...
        # Save as XISF
        if is_mono:
            rgb_image = np.stack([image[:, :, 0]] * 3, axis=-1).astype(np.float32)
        else:
            rgb_image = image.astype(np.float32)
        XISF.write(output_path, rgb_image, xisf_metadata=file_meta)
    else:
        # Save as 8-bit PNG
