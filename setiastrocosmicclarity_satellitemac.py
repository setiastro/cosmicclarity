...
def load_models(exe_dir, use_gpu=True):
    ...
    if torch.backends.mps.is_available() and use_gpu:
        ...
        detection_model1.load_state_dict(
            torch.load(os.path.join(exe_dir, "satellite_trail_detector_AI3.5.pth"), map_location=device)
        )
        ...
        detection_model2.load_state_dict(
            torch.load(os.path.join(exe_dir, "satellite_trail_detector_mobilenetv2.5.pth"), map_location=device)
        )
        ...
        removal_model.load_state_dict(
            torch.load(os.path.join(exe_dir, "satelliteremovalAI3.5.pth"), map_location=device)
        )
