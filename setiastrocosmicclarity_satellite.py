...
def load_models(exe_dir, use_gpu=True):
    ...
    if torch.cuda.is_available() and use_gpu:
        ...
        detection_model1 = load_model_weights(
            detection_model1,
            os.path.join(exe_dir, "satellite_trail_detector_AI3.5.pth"),
            device
        )
        ...
        detection_model2 = load_model_weights(
            detection_model2,
            os.path.join(exe_dir, "satellite_trail_detector_mobilenetv2.5.pth"),
            device
        )
        ...
        removal_model = load_model_weights(
            removal_model,
            os.path.join(exe_dir, "satelliteremovalAI3.5.pth"),
            device
        )
        removal_model.eval()
        ...
        detection_model1 = ort.InferenceSession(
            os.path.join(exe_dir, "satellite_trail_detector_AI3.5.onnx"),
            providers=["DmlExecutionProvider"]
        )
        detection_model2 = ort.InferenceSession(
            os.path.join(exe_dir, "satellite_trail_detector_mobilenetv2.5.onnx"),
            providers=["DmlExecutionProvider"]
        )
        removal_model = ort.InferenceSession(
            os.path.join(exe_dir, "satelliteremovalAI3.5.onnx"),
            providers=["DmlExecutionProvider"]
        )
