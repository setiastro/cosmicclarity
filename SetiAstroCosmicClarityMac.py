...
# Set models to evaluation mode and map them to the selected device (MPS or CPU)
...
def split_image_into_chunks_with_overlap(image, chunk_size, overlap):
    ...
    for i in range(0, height, step_size):
        for j in range(0, width, step_size):
            ...
            is_edge = i == 0 or j == 0 or (i + chunk_size >= height) or (j + chunk_size >= width)
...
def sharpen_image(image_path, sharpening_mode, nonstellar_strength, stellar_amount):
    ...
    if nonstellar_strength is not None:
        for idx, (chunk, i, j, is_edge) in enumerate(tqdm(chunks, desc=f"Processing non-stellar chunks for {os.path.basename(image_path)}", leave=False)):
    ...
    if sharpening_mode == "Stellar Only" or sharpening_mode == "Both":
        for idx, (chunk, i, j, is_edge) in enumerate(tqdm(chunks, desc=f"Processing stellar chunks for {os.path.basename(image_path)}", leave=False)):
            chunk_tensor = torch.tensor(chunk).unsqueeze(0).unsqueeze(0).to(device)
            with torch.no_grad():
