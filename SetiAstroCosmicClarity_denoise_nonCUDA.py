...
def load_models(exe_dir, use_gpu=True):
    """
    Load denoise models with support for DirectML and CPU fallback.

    Args:
        exe_dir (str): Path to the executable directory.
        use_gpu (bool): Whether to use GPU acceleration (DirectML).
    ...
    if use_gpu and "DmlExecutionProvider" in ort.get_available_providers():
        # Attempt to load ONNX models with DirectML
        print("Attempting to use DirectML for ONNX Runtime.")
        device = "DirectML"
        ...
        try:
            ...
            print("ONNX model loaded successfully with DirectML.")
        except Exception as e:
            print(f"Error loading ONNX model with DirectML: {e}")
    ...
    try:
        ...
        checkpoint_path = os.path.join(exe_dir, "deep_denoise_cnn_AI3.pth")
        checkpoint = torch.load(checkpoint_path, map_location=device)
        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            denoise_model.load_state_dict(checkpoint["model_state_dict"])
            print("PyTorch model state_dict loaded successfully.")
        else:
        ...
        raise RuntimeError("Failed to load the denoise model.") from e
